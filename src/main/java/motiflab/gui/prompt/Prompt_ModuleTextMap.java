 ...
 * @author kjetikl
 */
public class Prompt_ModuleTextMap extends Prompt {

    private ModuleTextMap data;
    ...
    public Prompt_ModuleTextMap(MotifLabGUI gui, String prompt, ModuleTextMap dataitem) {
        this(gui,prompt,dataitem,true);
    }

    public Prompt_ModuleTextMap(MotifLabGUI gui, String prompt, ModuleTextMap dataitem, boolean modal) {
        ...
        else data=new ModuleTextMap(gui.getGenericDataitemName(ModuleTextMap.class, null), "");
        setDataItemName(data.getName());
        setTitle(data.getDynamicType());
        SimpleDataPanelIcon icon=new SimpleDataPanelIcon(20, 20, SimpleDataPanelIcon.MODULE_TEXT_MAP_ICON,SimpleDataPanelIcon.NO_BORDER,gui.getVisualizationSettings());
    ...
    private void setupParseListPanel() {
        ...
        JLabel label=new JLabel("<html>Enter assignment pairs below separated by newlines.<br>Each pair should be in the format: <tt>Module = value</tt><br>The Module field can refer to a single module, a module collection,<br>a <i>range</i> of modules or a cluster within a module partition<br>(like so: <tt>PartitionName->ClusterName = value</tt>)<br>Wildcards (*) are allowed within module names.</html>");
    ...
    private void setupImportPanel() {
        ArrayList<DataFormat> dataformats=engine.getDataInputFormats(ModuleTextMap.class);
        //dataformats.remove(engine.getDataFormat("Plain"));
        importPanel=new LoadFromFilePanel(dataformats,gui,ModuleTextMap.class);
    }
    ...
    public boolean onOKPressed() {
        if (tabbedPanel.getSelectedComponent()==importPanel) {
            try {
                ...
                data=(ModuleTextMap)importPanel.loadData(data,ModuleTextMap.getType());
            ...
            try {
                data=ModuleTextMap.createModuleTextMapFromParameterString(data.getName(), Operation_new.FROM_PROPERTY_PREFIX+propertyName, null, engine);
            } catch (Exception e) {
                String exceptionText=e.getClass().getSimpleName();
                if (exceptionText.contains("ParseError") || exceptionText.contains("ExecutionError")) exceptionText="";
                else exceptionText+=":";
            ...
            ModuleTextMap parsedMap=null;
            try {
                parsedMap=ModuleTextMap.createModuleTextMapFromParameterString(data.getName(), text, notfound, engine);
          ...
          for (int i=0;i<table.getRowCount();i++) {
                ...
                else data.setValue(moduleName,valueText);               
           }
    ...
    public void setData(Data newdata) {
       if (newdata instanceof ModuleTextMap) data=(ModuleTextMap)newdata; 
    }     
