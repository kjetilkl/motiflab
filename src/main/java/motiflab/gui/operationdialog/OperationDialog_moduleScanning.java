...
import motiflab.engine.operations.Operation_moduleScanning;
import motiflab.external.ExternalProgram;
import motiflab.external.ModuleScanning;
 ...
 * @author kjetikl
 */
public class OperationDialog_moduleScanning extends OperationDialog {
    ...
    private String[] initialEditValues=new String[3]; // if set to other than NULL, this array contains the original algorithm, module sites trackname and names of additional results (the latter could be a comma-separated string) for a protocol line that is being edited
    ...
    public OperationDialog_moduleScanning(JFrame parent) {
        super(parent);
        //initComponents();
    }

    public OperationDialog_moduleScanning() {
        super();
    }

    public DefaultComboBoxModel getModuleScanningAlgorithms() {
    ...
    public void initComponents() {
        ...
        String algorithmName=(String)parameters.getParameter(Operation_moduleScanning.ALGORITHM);
        String sourceName=parameters.getSourceDataName();
        parameterSettings=(ParameterSettings)parameters.getParameter(Operation_moduleScanning.PARAMETERS);
        if (parameterSettings==null) {
            parameterSettings=new ParameterSettings();
            parameters.setParameter(Operation_moduleScanning.PARAMETERS,parameterSettings);
        }
    ...
    protected void setParameters() {
        ...
        if (algorithmComboBox.getItemCount()==0) setOK(false); // if no algorithms exists the OK button will effectively just cancel the dialog
            ...
            parameters.setParameter(Operation_moduleScanning.ALGORITHM, algorithmName);
            if (parametersPanel!=null) {
                parametersPanel.setParameters();
                parameters.setParameter(Operation_moduleScanning.PARAMETERS, parametersPanel.getParameterSettings());
            }
        ...
        if (additionalResultsPanel.isVisible()) { // this should ensure that a valid motif discovery program is selected
            ...
            if (!addResNamesString.isEmpty()) { // we have already checked that the number of names is correct (in "checkForErrors()" )
                ...
                parameters.setParameter(Operation_moduleScanning.ADDITIONAL_RESULTS, addResNamesString);
            ...
            } else parameters.removeParameter(Operation_moduleScanning.ADDITIONAL_RESULTS);
        } else parameters.removeParameter(Operation_moduleScanning.ADDITIONAL_RESULTS);      
    }
    ...
    private void initSequenceSubsetPanel() {
     ...
     private void initAlgorithmPanel(String sourceName, String algorithmName) {
        ...
        algorithmComboBox=new JComboBox(getModuleScanningAlgorithms());
        if (algorithmName==null){
            String defaultprogram=gui.getDefaultExternalProgram("modulescanning");
        ...
        algorithmComboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selected!=null) {
                    ...
                    ModuleScanning algorithm=(ModuleScanning)engine.getExternalProgram(selected);
    ...
    private void initSourceTargetPanel() {
        ...
        initialEditValues[0]=(String)parameters.getParameter(Operation_moduleScanning.ALGORITHM);
        initialEditValues[1]=targetName;
        initialEditValues[2]=(String)parameters.getParameter(Operation_moduleScanning.ADDITIONAL_RESULTS);        
    ...
    private JPanel getAlgorithmSettingsPanel(String algorithmName) {
        ModuleScanning algorithm=(ModuleScanning)engine.getExternalProgram(algorithmName);
        if (algorithm==null) {
            parametersPanel=null;
            return new JPanel();
        }
        String proxyModuleCollection=(String)parameters.getParameter(Operation_moduleScanning.PROXY_SOURCE_MODULECOLLECTION);
        Data modulecollection=(proxyModuleCollection!=null)?engine.getDataItem(proxyModuleCollection):null;
        if (modulecollection instanceof ModuleCollection) { // try to infuse proxy
            String mcolParameterName=algorithm.getNameOfFirstRegularParameterForType(ModuleCollection.class);
            if (mcolParameterName!=null) {
                parameterSettings.setParameter(mcolParameterName, modulecollection);
            }
        }        
