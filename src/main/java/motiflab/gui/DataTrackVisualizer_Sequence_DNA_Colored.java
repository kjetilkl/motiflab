 ...
 * @author kjetikl
 */
public class DataTrackVisualizer_Sequence_DNA_Colored extends DataTrackVisualizer_Sequence {
    
    /** Constructs a new DefaultDataTrackVisualizer */
    public DataTrackVisualizer_Sequence_DNA_Colored() {
        super();
    }
          ...
          for (int i=0;i<vpwidth;i++) {
             if (characterbuffer[i]!=null) {
                  switch(characterbuffer[i]) {
                      case 'A': case 'a': g.setPaint(colorA);break;
                      case 'C': case 'c': g.setPaint(colorC);break;
                      case 'G': case 'g': g.setPaint(colorG);break;
                      case 'T': case 't': g.setPaint(colorT);break;
                      default : g.setPaint(colorX);break;
                  }                        
