...
public class MenuScroller {
  public static final int DEFAULT_MENU_SIZE=20;
  private static final int DEFAULT_SCROLLING_INTERVAL=30;  
  //private JMenu menu;
  private JPopupMenu menu;
  private Component[] menuItems; // holds all menu items
  ...
  public MenuScroller(JMenu menu, int scrollCount) {
    this(menu, scrollCount, DEFAULT_SCROLLING_INTERVAL);
  }
  ...
  private class MenuScrollListener implements PopupMenuListener {
    ...
    private void setMenuItems() { // called when the menu is about to become visible the first time
      boolean firstTime=(menuItems==null);
      menuItems = menu.getComponents();
      if (firstTime) setProperSize(); // sets the width of all menu items to the largest width      
      ...
      if (menuItems.length > topFixedCount + scrollCount + bottomFixedCount) { // not all items will fit
         refreshMenu();      
      }
    }
    ...
    private void setProperSize() {
        ...
        for (Component item:menuItems) {
            ...
            if (dim.height>max.height) max.height=dim.height;
            if (dim.width>max.width) max.width=dim.width;            
        }
        for (Component item:menuItems) {
            item.setPreferredSize(max);         
        }
    }
  }
