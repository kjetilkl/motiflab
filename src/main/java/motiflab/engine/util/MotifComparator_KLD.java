...
/**
 * This compares two motifs using symmetric Kullback-Leibler Divergence
 *
 * @author kjetikl
 */
public class MotifComparator_KLD extends MotifComparator {
    ...
    @Override
    public String getName() {return "Kullback-Leibler Divergence";}

    @Override
    public String getAbbreviatedName() {return "KLD";}
    ...
    private double compareAtAlignment(double[][] largematrix,double[][] smallermatrix, int startA) {
        ...
        for (int i=start;i<end;i++) { //
            ...
            double colres=kld(largermatrixColumn,smallermatrixColumn);
            result+=colres;
        }
        return result;
    }
    ...
    /** Returns the symmetric Kullback-Leibler divergence for the two given columns */
    private double kld(double[] colA, double[] colB) {
        ...
        double firstTerm=columnA[0]*Math.log(columnA[0]/columnB[0])+columnA[1]*Math.log(columnA[1]/columnB[1])+columnA[2]*Math.log(columnA[2]/columnB[2])+columnA[3]*Math.log(columnA[3]/columnB[3]);
        double secondTerm=columnB[0]*Math.log(columnB[0]/columnA[0])+columnB[1]*Math.log(columnB[1]/columnA[1])+columnB[2]*Math.log(columnB[2]/columnA[2])+columnB[3]*Math.log(columnB[3]/columnA[3]);
