...
/**
 * This compares two motifs using Pearson's Correlation weighted by the information content in each column
 *
 * @author kjetikl
 */
public class MotifComparator_PearsonsCorrelationICWeighted extends MotifComparator {
    ...
    @Override
    public String getName() {return "Pearson's Correlation (weighted)";}

    @Override
    public String getAbbreviatedName() {return "Corr(w)";}    
    ...
    //    System.err.println("----------------  "+smallermotif.getName()+"  vs  "+largermotif.getName()+"  ---------------");
        ...
        if (start>=0) return new double[]{-10000,0}; // overlap is smaller than minimumOverlap (return 'default' bad value, large negative but NOT -Inf)
    ...
    private double compareAtAlignment(double[][] largematrix,double[][] smallermatrix, int startA) {
        ...
        for (int i=start;i<end;i++) { //
            ...
            double IClarger=Motif.calculateColumnIC(largermatrixColumn[0], largermatrixColumn[1], largermatrixColumn[2], largermatrixColumn[3], false);
            double ICsmaller=Motif.calculateColumnIC(smallermatrixColumn[0], smallermatrixColumn[1], smallermatrixColumn[2], smallermatrixColumn[3], false);
            //System.err.println("["+largermatrixpos+"]("+largermatrixColumn[0]+","+largermatrixColumn[1]+","+largermatrixColumn[2]+","+largermatrixColumn[3]+") vs ["+smallermatrixpos+"]("+smallermatrixColumn[0]+","+smallermatrixColumn[1]+","+smallermatrixColumn[2]+","+smallermatrixColumn[3]+") res="+colres);
            //double ICcorrection=(IClarger>ICsmaller)?IClarger:ICsmaller; // use the largest IC of the two aligned columns as "IC correction term"
            //result+=colres*(ICcorrection/2); // divide by 2 to normalize the IC correction
            result+=colres*(IClarger*ICsmaller/4);
        }
        return result;
    }
