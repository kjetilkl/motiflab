...
public class ModuleCRM extends Data implements BasicDataType {
    ...
    private ModuleCRM() {
        // only used when deserializing objects of the old Module type
    }
   ...
   /** Returns a list of names for all standard Motif properties such as "short name" and "Classification".
    ...
    // used when reading serialized objects of the old Module class from file
    public static ModuleCRM fromLegacy(Object legacy) throws Exception {
        Class<?> legacyClass = legacy.getClass();
        ModuleCRM result = new ModuleCRM();
        // Extract fields using reflection
        result.name = (String) legacyClass.getMethod("getName").invoke(legacy);
        result.singleMotifs = (ArrayList<ModuleMotif>) legacyClass.getMethod("getSingleMotifs").invoke(legacy);
        result.distance = (ArrayList<int[]>) legacyClass.getMethod("getDistance").invoke(legacy);
        result.maxLength = (int) legacyClass.getMethod("getMaxLength").invoke(legacy);
        result.ordered = (boolean) legacyClass.getMethod("isOrdered").invoke(legacy);
        result.properties = (HashMap<String, Object>) legacyClass.getMethod("getProperties").invoke(legacy);
        result.GOterms = (int[]) legacyClass.getMethod("getGOterms").invoke(legacy);
        return result;
    }   
