...
public class Operation_copy extends Operation {
    ...
    public boolean execute(OperationTask task) throws Exception {
        ...
        task.checkExecutionLock(); // checks to see if this task should suspend execution
        if (Thread.interrupted() || task.getStatus().equals(ExecutableTask.ABORTED)) throw new InterruptedException();
        if (originalData instanceof FeatureDataset && engine.getClient() instanceof org.motiflab.gui.MotifLabGUI && !engine.dataExists(targetName, null)) { // a small hack to copy visualization settings from source when creating a new target
