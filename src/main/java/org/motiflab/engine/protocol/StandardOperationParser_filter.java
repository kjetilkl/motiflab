 ...
 * @author kjetikl
 */
public class StandardOperationParser_filter extends StandardOperationParser {
    ...
    public OperationTask parse(String command) throws ParseError {
           ...
           Pattern pattern=Pattern.compile("^(([a-zA-Z_0-9-]+)\\s*=\\s*)?filter ([a-zA-Z_0-9-]+)?( where (.+))?(\\s*\\S.*)?");
           ...
           if (sourceName==null || sourceName.isEmpty()) throw new ParseError("Missing source data for filter operation");
