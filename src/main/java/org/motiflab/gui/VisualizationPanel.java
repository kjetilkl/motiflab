...
public class VisualizationPanel extends javax.swing.JScrollPane implements VisualizationSettingsListener, MouseWheelListener, Printable, Searchable {
    ...
    public static final String ACTION_MARK_REGION="Mark Region";    
...
//        this.addComponentListener(resizeListener);
    ...
    private void setupActions() {
        ...
        inputmap.put(KeyStroke.getKeyStroke(KeyEvent.VK_M, InputEvent.SHIFT_DOWN_MASK), ACTION_MARK_REGION);
        actionmap.put(ACTION_MARK_REGION, new AbstractAction() {public void actionPerformed(ActionEvent e) {toggleRegionMark();} });        
    ...
    /** Toggles the "mark" property on the currently selected region
     */
    private void toggleRegionMark() {
        Region current=getCurrentRegion();       
        if (current!=null) {
             FeatureSequenceData seq=current.getParent();
             if (seq==null) return;
             SequenceVisualizer viz=getSequenceVisualizer(seq.getSequenceName());
             if (viz!=null) viz.toggleRegionMark(current);
        }
    }    
