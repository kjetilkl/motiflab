...
public class VisualizationSettings implements ListDataListener, ExtendedDataListener, Serializable {
   ...
   * 
    ...
    /**
     * A convenience method to fetch the alpha value for the foreground color of a dataset
     * @param datasetname The name of the dataset
     * @return The alpha value (0 is totally transparent and 255 is totally opaque)
     */
    public int getForeGroundAlpha(String datasetname) {
        Color color=getForeGroundColor(datasetname);
        return color.getAlpha();        
    }
    
    /**
     * A convenience method to set the alpha value for the foreground color of a dataset
     * @param datasetname The name of the dataset
     * @param alpha The transparency value from 0 (totally transparent) to 255 (totally opaque)
     */
    public void setForeGroundAlpha(String datasetname, int alpha) {
        if (datasetname==null) return;               
        Color color=getForeGroundColor(datasetname);
        Color newColor = new Color(color.getRed(),color.getGreen(),color.getBlue(),alpha);
        setForeGroundColor(datasetname,newColor);
    }    
    
    /**
     * A convenience method to set the alpha value for the foreground color for multiple datasets
     * @param datasetnames The names of the datasets
     * @param alpha The transparency value from 0 (totally transparent) to 255 (totally opaque)
     */
    public void setForeGroundAlpha(String datasetnames[], int alpha) { 
        for (String datasetname:datasetnames) {
            Color color=getForeGroundColor(datasetname);
            Color newColor = new Color(color.getRed(),color.getGreen(),color.getBlue(),alpha);
            storeSetting(datasetname+"."+FOREGROUND_COLOR,newColor);
            storeSetting(datasetname+"."+COLORGRADIENT,null); // invalidate gradients
        }
        redraw();
    }    
