 ...
 * @author kjetikl
 */
public class Prompt_MotifNumericMap extends Prompt {
    
    private MotifNumericMap data;
    ...
    public Prompt_MotifNumericMap(MotifLabGUI gui, String prompt, MotifNumericMap dataitem) {
        this(gui,prompt,dataitem,true);
    }
    
    public Prompt_MotifNumericMap(MotifLabGUI gui, String prompt, MotifNumericMap dataitem, boolean modal) {
        ...
        else data=new MotifNumericMap(gui.getGenericDataitemName(MotifNumericMap.class, null), 0);
        setDataItemName(data.getName());
        setTitle("Motif Numeric Map");
        SimpleDataPanelIcon icon=new SimpleDataPanelIcon(20, 20, SimpleDataPanelIcon.MOTIF_NUMERIC_MAP_ICON,SimpleDataPanelIcon.NO_BORDER,gui.getVisualizationSettings());
    ...
    private void setupParseListPanel() {
        ...
        JLabel label=new JLabel("<html>Enter assignment pairs below separated by newline or comma.<br>Each pair should be in the format: Motif = value<br>The Motif field can refer to a single motif or a motif collection.<br>Wildcards (*) are allowed within motif names.</html>");
     ...
     private void setupFromTrackPanel() {
        ...
        internal.add(new JLabel("Motif Track  "),constraints);
        ...
        ArrayList<Data> motifTracks=engine.getAllDataItemsOfType(RegionDataset.class);
        for (Data motiftrack:motifTracks) {
            if (((RegionDataset)motiftrack).isMotifTrack()) tracksmodel.addElement(motiftrack.getName());
        }
        ...
        fromTrackPropertyCombobox=new JComboBox(MotifNumericMap.getFromTrackPropertyNames());
    ...
    private void setupFromPropertyPanel() {
        ...
        fromPropertyCombobox=new JComboBox(Motif.getAllNumericProperties(true,engine));
    ...
    private void setupImportPanel() {
        ArrayList<DataFormat> dataformats=engine.getDataInputFormats(MotifNumericMap.class);
        //dataformats.remove(engine.getDataFormat("Plain"));
        importPanel=new LoadFromFilePanel(dataformats,gui,MotifNumericMap.class);
    }
    ...
    private void setupDistributionPanel() {
        ...
        ArrayList<String> collections=engine.getNamesForAllDataItemsOfType(MotifCollection.class);
        for (Data partition:engine.getAllDataItemsOfType(MotifPartition.class)) {
            for (String cluster:((MotifPartition)partition).getClusterNames()) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (!colName.isEmpty()) {
                    ...
                    if (colName.contains(".")) {
                        ...
                        if (part instanceof MotifPartition) selcol=((MotifPartition)part).getClusterAsMotifCollection(split[1], engine);

                    } else selcol=engine.getDataItem(colName);
                    if (selcol instanceof MotifCollection) {
                        values=new double[((MotifCollection)selcol).size()];
                        for (int i=0;i<values.length;i++) values[i]=data.getValue(((MotifCollection)selcol).getMotifNameByIndex(i));
                    }
                }
    ...
    public boolean onOKPressed() {    
        if (tabbedPanel.getSelectedComponent()==importPanel) {
            try {
                ...
                data=(MotifNumericMap)importPanel.loadData(data,MotifNumericMap.getType());
            ...
            try {
                data=MotifNumericMap.createMotifNumericMapFromParameterString(data.getName(), Operation_new.FROM_PROPERTY_PREFIX+propertyName, null, engine, null);
            } catch (Exception e) {
                String exceptionText=e.getClass().getSimpleName();
                if (exceptionText.contains("ParseError") || exceptionText.contains("ExecutionError")) exceptionText="";
                else exceptionText+=":";
            ...
            try {
                ...
                int property=-1;
                String[] trackPropertyNames=MotifNumericMap.getFromTrackPropertyNames();
                for (int p=0;p<trackPropertyNames.length;p++) {
                    if (propertyName.equalsIgnoreCase(trackPropertyNames[p])) property=p;
                }
                if (property<0) throw new ExecutionError("Unknown property: "+propertyName);
                ...
                if (withinRegionsName!=null && !withinRegionsName.isEmpty() && withinRegionsName.equals(trackName)) throw new ExecutionError("It is not advisable to use same motif track to specify 'within' limits");
                ...
                } else { // the prompt is shown in response to an "Add New Motif Numeric Map" request. Just set the basic parameters needed for a command line. A "new" operation task will be run later to create the finished data object and this avoids creating the map twice
                    data=new MotifNumericMap(data.getName(),0);
            ...
            MotifNumericMap parsedMap=null;
            try {
                parsedMap=MotifNumericMap.createMotifNumericMapFromParameterString(data.getName(), text, notfound, engine, null);
          ...
          for (int i=0;i<table.getRowCount();i++) {
                ...
                if (valueText.isEmpty()) data.removeValue(motifName);
                    ...
                    data.setValue(motifName,value);
                } catch (NumberFormatException e) {
                   reportErrorInTable("Value for '"+motifName+"' is not numeric:  "+valueText,i);
                   return false;
                }
           }
           try {
    ...
    public void setData(Data newdata) {
       if (newdata instanceof MotifNumericMap) data=(MotifNumericMap)newdata; 
    }  
    
    @Override
    public Data createDataObject(final Object[] parameters, OperationTask task) throws Exception {
        if (parameters==null || parameters.length==0) return new MotifNumericMap("dummy",0);
        if (((String)parameters[0]).equals("track")) {
            String targetName=(String)parameters[1];
            String parameter=(String)parameters[2];
            return MotifNumericMap.createMotifNumericMapFromParameterString(targetName, Operation_new.FROM_TRACK_PREFIX+parameter, null, engine, task);
        } else return new MotifNumericMap("dummy",0);
    } 
