 ...
 * @author kjetikl
 */
public class MotifBrowserPanel extends JPanel implements DataListener {
    ...
    private String[] columnHeaders=new String[] {" ","Motif","Filter","Logo"}; // the name of the Filter column will be changed later on 
    ...
    private static final int MOTIF_COLUMN=1;
    ...
    public MotifBrowserPanel(MotifLabGUI gui, MotifCollection motifcollection, boolean modal, boolean allowCreateCollection) {        
        super();
        this.gui=gui;
        this.engine=gui.getEngine();
        this.motifcollection=motifcollection;
        this.isModal=modal;
        if (MotifBrowserPanel.blackbullet==null) {
            ...
            MotifBrowserPanel.blackbullet=new ImageIcon(blackBulletURL); 
            MotifBrowserPanel.greenbullet=new ImageIcon(greenBulletURL);     
        }
    ...
    private void setupManualEntryPanel(boolean allowCreateCollection) { 
        ...
        if (this.motifcollection!=null) {
            ...
            showOnlyCollectionMembersCheckbox.setToolTipText("Click to toggle between showing only motifs included in the collection (green) or all motifs (black)");
        ...
        columnModel.getColumn(MOTIF_COLUMN).setCellRenderer(new CellRenderer_Motif());
        ...
        columnModel.getColumn(LOGO_COLUMN).setCellRenderer(logorenderer);
        ((TableRowSorter)manualSelectionTable.getRowSorter()).setComparator(FILTER_COLUMN, new Sorter());              
        ((TableRowSorter)manualSelectionTable.getRowSorter()).setComparator(LOGO_COLUMN, new MotifLogoComparator());              
    ...
    private void updateCountLabelText() {
        ...
        int included = countSelectedMotifs();
        int matchingAgainst = (showOnlyMembersDefault)?included:total;
        if (motifcollection!=null) numberOfMotifsShowingLabel.setText("<html>Matching: "+manualSelectionTable.getRowCount()+" of "+matchingAgainst+"<br>Included: "+included+" of "+total+"</html>");
...
private class ManualSelectionTableModel extends AbstractTableModel { 
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        if (columnIndex==SELECTED_COLUMN) return checked.get(rowIndex);
            ...
            else if (filterColumn.equals("Names")) return getMotifDisplayName((Motif)motifs.get(rowIndex)); // this is not a property of the Motif itself but a dynamic property created only for this table/filter
            else if (filterColumn.equals("Names + Factors")) return getMotifNameAndFactors((Motif)motifs.get(rowIndex));  // this is not a property of the Motif itself but a dynamic property created only for this table/filter
...
private class Filter extends RowFilter<Object,Object> {
    ...
    public boolean include(Entry<? extends Object, ? extends Object> entry) {
     if (filterString==null && !showOnlyCollectionMembers) return true; // no filtering at all applied. Note that the filterString will be set to NULL if it is empty (or just whitespace)        
     ...
     if (Number.class.isAssignableFrom(classtype)) {
         ...
         boolean showContains=!(comparator.equals("<>") || comparator.equals("!~"));
