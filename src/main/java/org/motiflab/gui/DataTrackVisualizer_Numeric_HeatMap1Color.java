 ...
 * @author kjetikl
 */
public class DataTrackVisualizer_Numeric_HeatMap1Color extends DataTrackVisualizer_Numeric {
       
    public DataTrackVisualizer_Numeric_HeatMap1Color() {
        super();
    } 
    ...
    public String getGraphTypeName() {
        return "Heatmap (1 color)";
    }
    
    @Override
    public SimpleDataPanelIcon getGraphTypeIcon() {
        return new SimpleDataPanelIcon(20,20,SimpleDataPanelIcon.NUMERIC_TRACK_GRADIENT_ICON,null);
    }       
              ...
              if (value instanceof Double) { // bargraph or gradient. Instead of drawing a single solid line, finetune the "tip" of the bar with a gradient color to get more precision
                    if (drawBaseLine) {
                       ...
                       if (val==baselinevalue) continue;
                           ...
                           drawBase(graphics,graphicsXoffset+i, graphicsYoffset, aboveHeight-1);
                       } else {
                           double relativeValue=Math.abs((baselinevalue-val)/directionalMaximum);
                           graphics.setColor(gradient.getColor(relativeValue));
                           drawBase(graphics,graphicsXoffset+i, graphicsYoffset+aboveHeight+1, height-aboveHeight-2);
                       }
